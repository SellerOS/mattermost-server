...
type SqlUserStore struct {
	...
	// usersQuery is a starting point for all queries that return one or more UserIms.
...
func NewSqlUserStore(sqlStore SqlStore, metrics einterfaces.MetricsInterface) store.UserStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		table := db.AddTableWithName(model.UserIms{}, "UserIms").SetKeys(true, "Id")
...
func (us SqlUserStore) CreateIndexesIfNotExists() {
	us.CreateIndexIfNotExists("idx_users_email", "UserIms", "Email")
	us.CreateIndexIfNotExists("idx_users_update_at", "UserIms", "UpdateAt")
	...
	us.CreateFullTextIndexIfNotExists("idx_users_names_txt", "UserIms", strings.Join(USER_SEARCH_TYPE_NAMES, ", "))
	us.CreateFullTextIndexIfNotExists("idx_users_names_no_full_name_txt", "UserIms", strings.Join(USER_SEARCH_TYPE_NAMES_NO_FULL_NAME, ", "))
}

func (us SqlUserStore) Save(user *model.UserIms) store.StoreChannel {
...
func (us SqlUserStore) Update(user *model.UserIms, trustedUpdateData bool) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		user.PreUpdate()
		...
		if oldUserResult, err := us.GetMaster().Get(model.UserIms{}, user.ClientId); err != nil {
			result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.finding.app_error", nil, "user_id="+user.ClientId+", "+err.Error(), http.StatusInternalServerError)
		} else if oldUserResult == nil {
			result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.find.app_error", nil, "user_id="+user.ClientId, http.StatusBadRequest)
		} else {
			oldUser := oldUserResult.(*model.UserIms)
			...
			if user.IsOAuthUser() {
				...
				if user.Username != oldUser.Username ||
					user.Email != oldUser.Email {
					result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.can_not_change_ldap.app_error", nil, "user_id="+user.ClientId, http.StatusBadRequest)
					return
				}
			}
			...
			if count, err := us.GetMaster().Update(user); err != nil {
				if IsUniqueConstraintError(err, []string{"Email", "users_email_key", "idx_users_email_unique"}) {
					result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.email_taken.app_error", nil, "user_id="+user.ClientId+", "+err.Error(), http.StatusBadRequest)
				} else if IsUniqueConstraintError(err, []string{"Username", "users_username_key", "idx_users_username_unique"}) {
					result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.username_taken.app_error", nil, "user_id="+user.ClientId+", "+err.Error(), http.StatusBadRequest)
				} else {
					result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.updating.app_error", nil, "user_id="+user.ClientId+", "+err.Error(), http.StatusInternalServerError)
				}
			} else if count != 1 {
				result.Err = model.NewAppError("SqlUserStore.Update", "store.sql_user.update.app_error", nil, fmt.Sprintf("user_id=%v, count=%v", user.ClientId, count), http.StatusInternalServerError)
			} else {
				user.Sanitize(map[string]bool{})
				oldUser.Sanitize(map[string]bool{})
				result.Data = [2]*model.UserIms{user, oldUser}
			}
		}
	})
}
...
func (us SqlUserStore) UpdateLastPictureUpdate(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		//if _, err := us.GetMaster().Exec("UPDATE UserIms SET LastPictureUpdate = :Time, UpdateAt = :Time WHERE ClientId = :UserId", map[string]interface{}{"Time": curTime, "UserId": userId}); err != nil {
...
func (us SqlUserStore) ResetLastPictureUpdate(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		//if _, err := us.GetMaster().Exec("UPDATE UserIms SET LastPictureUpdate = :Time, UpdateAt = :Time WHERE ClientId = :UserId", map[string]interface{}{"Time": 0, "UserId": userId}); err != nil {
...
func (us SqlUserStore) UpdateUpdateAt(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		curTime := model.GetMillis()
		if _, err := us.GetMaster().Exec("UPDATE UserIms SET UpdateAt = :Time WHERE ClientId = :UserId", map[string]interface{}{"Time": curTime, "UserId": userId}); err != nil {
...
func (us SqlUserStore) UpdatePassword(userId, hashedPassword string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := us.GetMaster().Exec("UPDATE user_login SET passwordEncryption = :Password, modified = :UpdateAt WHERE ClientId = :UserId", map[string]interface{}{"Password": hashedPassword, "UpdateAt": updateAt, "UserId": userId}); err != nil {
...
func (us SqlUserStore) UpdateFailedPasswordAttempts(userId string, attempts int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		//if _, err := us.GetMaster().Exec("UPDATE UserIms SET FailedAttempts = :FailedAttempts WHERE ClientId = :UserId", map[string]interface{}{"FailedAttempts": attempts, "UserId": userId}); err != nil {
...
func (us SqlUserStore) UpdateMfaSecret(userId, secret string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := us.GetMaster().Exec("UPDATE UserIms SET MfaSecret = :Secret, UpdateAt = :UpdateAt WHERE ClientId = :UserId", map[string]interface{}{"Secret": secret, "UpdateAt": updateAt, "UserId": userId}); err != nil {
...
func (us SqlUserStore) UpdateMfaActive(userId string, active bool) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := us.GetMaster().Exec("UPDATE UserIms SET MfaActive = :Active, UpdateAt = :UpdateAt WHERE ClientId = :UserId", map[string]interface{}{"Active": active, "UpdateAt": updateAt, "UserId": userId}); err != nil {
...
func (us SqlUserStore) Get(id string) (*model.UserIms, *model.AppError) {
...
func (us SqlUserStore) GetUserInfo(clientId string) (*model.UserInfo, *model.AppError) {
	user := model.UserInfo{}

	if err := us.GetReplica().SelectOne(&user, "SELECT * FROM user WHERE clientId = :clientId", map[string]interface{}{"clientId": clientId}); err != nil {
		return nil, model.NewAppError("SqlUserStore.GetInfo", "store.sql_user.get_user_info.app_error", nil, err.Error(), http.StatusInternalServerError)
	}

	return &user, nil
}

func (us SqlUserStore) GetUserLogin(clientId string) (*model.UserLogin, *model.AppError) {
	user := model.UserLogin{}

	if err := us.GetReplica().SelectOne(&user, "SELECT * FROM user_login WHERE clientId = :clientId", map[string]interface{}{"clientId": clientId}); err != nil {
		return nil, model.NewAppError("SqlUserStore.GetUserLogin", "store.sql_user.get_user_login.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (s SqlUserStore) GetEtagForAllProfiles() store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		updateAt, err := s.GetReplica().SelectInt("SELECT UpdateAt FROM UserIms ORDER BY UpdateAt DESC LIMIT 1")
		if err != nil {
...
func (s SqlUserStore) GetEtagForProfiles(teamId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		updateAt, err := s.GetReplica().SelectInt("SELECT UpdateAt FROM UserIms, TeamMembers WHERE TeamMembers.TeamId = :TeamId AND UserIms.ClientId = TeamMembers.UserId ORDER BY UpdateAt DESC LIMIT 1", map[string]interface{}{"TeamId": teamId})
		if err != nil {
			result.Data = fmt.Sprintf("%v.%v", model.CurrentVersion, model.GetMillis())
		} else {
			result.Data = fmt.Sprintf("%v.%v", model.CurrentVersion, updateAt)
...
func (us SqlUserStore) GetProfiles(options *model.UserGetOptions) store.StoreChannel {
	...
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			Join("TeamMembers tm ON ( tm.UserId = u.ClientId AND tm.DeleteAt = 0 )").
...
func (us SqlUserStore) InvalidateProfilesInChannelCacheByUser(userId string) {
	...
	for _, key := range keys {
		if cacheItem, ok := profilesInChannelCache.Get(key); ok {
			userMap := cacheItem.(map[string]*model.UserIms)
			if _, userInCache := userMap[userId]; userInCache {
				profilesInChannelCache.Remove(key)
				if us.metrics != nil {
					us.metrics.IncrementMemCacheInvalidationCounter("Profiles in Channel - Remove by UserIms")
				}
			}
		}
	}
...
func (us SqlUserStore) GetProfilesInChannel(channelId string, offset int, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			Join("ChannelMembers cm ON ( cm.UserId = u.ClientId )").
...
func (us SqlUserStore) GetProfilesInChannelByStatus(channelId string, offset int, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			Join("ChannelMembers cm ON ( cm.UserId = u.ClientId )").
			LeftJoin("Status s ON ( s.UserId = u.ClientId )").
			Where("cm.ChannelId = ?", channelId).
			OrderBy(`
				CASE s.Status
					WHEN 'online' THEN 1
	...
	return store.Do(func(result *store.StoreResult) {
		if allowFromCache {
			if cacheItem, ok := profilesInChannelCache.Get(channelId); ok {
				...
				result.Data = cacheItem.(map[string]*model.UserIms)
				return
			} else {
				if us.metrics != nil {
...
func (us SqlUserStore) GetProfilesNotInChannel(teamId string, channelId string, offset int, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			Join("TeamMembers tm ON ( tm.UserId = u.ClientId AND tm.DeleteAt = 0 AND tm.TeamId = ? )", teamId).
			LeftJoin("ChannelMembers cm ON ( cm.UserId = u.ClientId AND cm.ChannelId = ? )", channelId).
...
func (us SqlUserStore) GetProfilesByUsernames(usernames []string, teamId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if teamId != "" {
			query = query.Join("TeamMembers tm ON (tm.UserId = u.ClientId AND tm.TeamId = ?)", teamId)
		}
...
func (us SqlUserStore) GetRecentlyActiveUsersForTeam(teamId string, offset, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			...
			Join("TeamMembers tm ON (tm.UserId = u.ClientId AND tm.TeamId = ?)", teamId).
			Join("Status s ON (s.UserId = u.ClientId)").
		...
		queryString, args, err := query.ToSql()
		if err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetRecentlyActiveUser", "store.sql_user.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}

		var users []*UserWithLastActivityAt
		if _, err := us.GetReplica().Select(&users, queryString, args...); err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetRecentlyActiveUser", "store.sql_user.get_recently_active_users.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}

		userList := []*model.UserIms{}

		for _, userWithLastActivityAt := range users {
			u := userWithLastActivityAt.UserIms
			u.Sanitize(map[string]bool{})
...
func (us SqlUserStore) GetProfileByIds(userIds []string, allowFromCache bool) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		for _, u := range users {
			...
			cpy := &model.UserIms{}
			*cpy = *u
			profileByIdsCache.AddWithExpiresInSecs(cpy.ClientId, cpy, PROFILE_BY_IDS_CACHE_SEC)
		}
...
func (us SqlUserStore) GetUserLoginByEmail(email string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		email = strings.ToLower(email)

		user := model.UserLogin{}
		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM user_login WHERE email = :email", map[string]interface{}{"email": email}); err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetByEmail", "store.sql_user.get_by_email.app_error", nil, err.Error(), http.StatusInternalServerError)
		}
...
func (us SqlUserStore) VerifyEmail(userId, email string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		curTime := model.GetMillis()
		if _, err := us.GetMaster().Exec("UPDATE UserIms SET Email = :email, UpdateAt = :Time WHERE ClientId = :UserId", map[string]interface{}{"email": email, "Time": curTime, "UserId": userId}); err != nil {
...
func (us SqlUserStore) PermanentDelete(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if _, err := us.GetMaster().Exec("DELETE FROM UserIms WHERE ClientId = :UserId", map[string]interface{}{"UserId": userId}); err != nil {
...
func (us SqlUserStore) Count(options model.UserCountOptions) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := sq.Select("COUNT(UserIms.Id)").From("UserIms")

		if !options.IncludeDeleted {
			query = query.Where("UserIms.DeleteAt = 0")
		}

		if options.IncludeBotAccounts {
			if options.ExcludeRegularUsers {
				query = query.Join("Bots ON UserIms.ClientId = Bots.UserId")
			}
		} else {
			query = query.LeftJoin("Bots ON UserIms.ClientId = Bots.UserId").Where("Bots.UserId IS NULL")
			if options.ExcludeRegularUsers {
		...
		if options.TeamId != "" {
			query = query.LeftJoin("TeamMembers ON UserIms.ClientId = TeamMembers.UserId").Where("TeamMembers.TeamId = ? AND TeamMembers.DeleteAt = 0", options.TeamId)
		}
...
func (us SqlUserStore) AnalyticsActiveCount(timePeriod int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if err != nil {
			result.Err = model.NewAppError("SqlUserStore.AnalyticsDailyActiveUserIms", "store.sql_user.analytics_daily_active_users.app_error", nil, err.Error(), http.StatusInternalServerError)
		} else {
			result.Data = v
		}
	})
...
func (us SqlUserStore) Search(teamId string, term string, options *model.UserSearchOptions) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if teamId != "" {
			query = query.Join("TeamMembers tm ON ( tm.UserId = u.ClientId AND tm.DeleteAt = 0 AND tm.TeamId = ? )", teamId)
		}
...
func (us SqlUserStore) SearchNotInTeam(notInTeamId string, term string, options *model.UserSearchOptions) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			LeftJoin("TeamMembers tm ON ( tm.UserId = u.ClientId AND tm.DeleteAt = 0 AND tm.TeamId = ? )", notInTeamId).
...
func (us SqlUserStore) SearchInChannel(channelId string, term string, options *model.UserSearchOptions) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			Join("ChannelMembers cm ON ( cm.UserId = u.ClientId AND cm.ChannelId = ? )", channelId).
			OrderBy("Username ASC").
			Limit(uint64(options.Limit))
...
func (us SqlUserStore) AnalyticsGetInactiveUsersCount() store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if count, err := us.GetReplica().SelectInt("SELECT COUNT(Id) FROM UserIms WHERE DeleteAt > 0"); err != nil {
...
func (us SqlUserStore) AnalyticsGetSystemAdminCount() store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if count, err := us.GetReplica().SelectInt("SELECT count(*) FROM UserIms WHERE Roles LIKE :Roles and DeleteAt = 0", map[string]interface{}{"Roles": "%system_admin%"}); err != nil {
...
func (us SqlUserStore) GetProfilesNotInTeam(teamId string, offset int, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		query := us.usersQuery.
			LeftJoin("TeamMembers tm ON ( tm.UserId = u.ClientId AND tm.DeleteAt = 0 AND tm.TeamId = ? )", teamId).
			Where("tm.UserId IS NULL").
			OrderBy("u.Username ASC").
			Offset(uint64(offset)).Limit(uint64(limit))
...
func (us SqlUserStore) ClearAllCustomRoleAssignments() store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		for {
			...
			var users []*model.UserIms
			if _, err := transaction.Select(&users, "SELECT * from UserIms WHERE ClientId > :Id ORDER BY Id LIMIT 1000", map[string]interface{}{"Id": lastUserId}); err != nil {
			...
			for _, user := range users {
				...
				if newRolesString != user.Roles {
					if _, err := transaction.Exec("UPDATE UserIms SET Roles = :Roles WHERE ClientId = :Id", map[string]interface{}{"Roles": newRolesString, "Id": user.ClientId}); err != nil {
...
func (us SqlUserStore) InferSystemInstallDate() store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		createAt, err := us.GetReplica().SelectInt("SELECT CreateAt FROM UserIms WHERE CreateAt IS NOT NULL ORDER BY CreateAt ASC LIMIT 1")
		if err != nil {
...
func (us SqlUserStore) GetUsersBatchForIndexing(startTime, endTime int64, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		for _, user := range users {
			userMap[user.ClientId] = &model.UserForIndexing{
				Id:          user.ClientId,
