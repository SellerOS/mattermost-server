...
func testGroupCreateOrRestoreMember(t *testing.T, ss store.Store) {
	...
	require.Equal(t, d2.UserId, user.ClientId)
...
func testPendingAutoAddTeamMembers(t *testing.T, ss store.Store) {
	...
	require.Equal(t, user.ClientId, teamMembers[0].UserID)
...
func testPendingAutoAddChannelMembers(t *testing.T, ss store.Store) {
	...
	require.Equal(t, user.ClientId, channelMembers[0].UserID)
...
func testPendingTeamMemberRemovals(t *testing.T, ss store.Store) {
	...
	require.Equal(t, data.UserC.ClientId, teamMembers[0].UserId)

	res = <-ss.Group().DeleteMember(data.Group.Id, data.UserB.ClientId)
	require.Nil(t, res.Err)
	...
	require.Contains(t, userIDs, data.UserB.ClientId)
	require.Contains(t, userIDs, data.UserC.ClientId)
	require.Equal(t, data.ConstrainedTeam.Id, teamMembers[0].TeamId)
	require.Equal(t, data.ConstrainedTeam.Id, teamMembers[1].TeamId)

	res = <-ss.Group().DeleteMember(data.Group.Id, data.UserA.ClientId)
	require.Nil(t, res.Err)
	...
	res = <-ss.Team().RemoveMember(data.ConstrainedTeam.Id, data.UserA.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Team().RemoveMember(data.ConstrainedTeam.Id, data.UserB.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Team().RemoveMember(data.ConstrainedTeam.Id, data.UserC.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Channel().RemoveMember(data.ConstrainedChannel.Id, data.UserA.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Channel().RemoveMember(data.ConstrainedChannel.Id, data.UserB.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Channel().RemoveMember(data.ConstrainedChannel.Id, data.UserC.ClientId)
	require.Nil(t, res.Err)
}
...
func testPendingChannelMemberRemovals(t *testing.T, ss store.Store) {
	...
	require.Equal(t, data.UserC.ClientId, channelMembers[0].UserId)
...
func pendingMemberRemovalsDataSetup(t *testing.T, ss store.Store) *removalsData {
	...
	res = <-ss.Group().CreateOrRestoreMember(group.Id, userB.ClientId)
	require.Nil(t, res.Err)
	...
	userIDTeamIDs := [][]string{
		{userA.ClientId, teamConstrained.Id},
		{userB.ClientId, teamConstrained.Id},
		{userC.ClientId, teamConstrained.Id},
		{userA.ClientId, teamUnconstrained.Id},
		{userB.ClientId, teamUnconstrained.Id},
		{userC.ClientId, teamUnconstrained.Id},
	}
	...
	userIDChannelIDs := [][]string{
		{userA.ClientId, channelConstrained.Id},
		{userB.ClientId, channelConstrained.Id},
		{userC.ClientId, channelConstrained.Id},
		{userA.ClientId, channelUnconstrained.Id},
		{userB.ClientId, channelUnconstrained.Id},
		{userC.ClientId, channelUnconstrained.Id},
	}
