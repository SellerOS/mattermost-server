...
func testUserStoreGet(t *testing.T, ss store.Store) {
	...
	t.Run("fetch user 1", func(t *testing.T) {
		actual, err := ss.User().Get(u1.ClientId)
		require.Nil(t, err)
	...
	t.Run("fetch user 2, also a bot", func(t *testing.T) {
		actual, err := ss.User().Get(u2.ClientId)
		require.Nil(t, err)
...
func testUserStoreGetTeamGroupUsers(t *testing.T, ss store.Store) {
	...
	res = <-ss.Group().CreateOrRestoreMember(groupA.Id, userGroupA.ClientId)
	require.Nil(t, res.Err)
	res = <-ss.Group().CreateOrRestoreMember(groupB.Id, userGroupB.ClientId)
	require.Nil(t, res.Err)
	...
	res = <-ss.Team().RemoveMember(team.Id, userGroupA.ClientId)
	require.Nil(t, res.Err)
...
func testUserStoreGetChannelGroupUsers(t *testing.T, ss store.Store) {
	...
	// delete team membership of allowed user
	res = <-ss.Channel().RemoveMember(channel.Id, userGroupA.ClientId)
	require.Nil(t, res.Err)
